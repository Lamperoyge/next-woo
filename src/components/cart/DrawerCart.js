  ...
  return (
    <div>
      {isLoaded && (
        <aside
          ref={ref}
          className={`transform top-0 right-0 w-96 bg-white fixed h-full overflow-auto ${
            isLoaded ? 'ease-in-out transition-all duration-300' : ''
          } z-30 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
