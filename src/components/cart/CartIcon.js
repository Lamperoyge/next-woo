import { connect } from 'react-redux';
import { openCart } from 'redux/cart/cart.actions';
function Cart({ count, openCart }) {
  return (
    <div onClick={openCart} className='cursor-pointer'>
      <div className='relative'>
        <svg
          width={34}
          height={31}
          viewBox='0 0 34 31'
          xmlns='http://www.w3.org/2000/svg'
          xmlnsXlink='http://www.w3.org/1999/xlink'
          className='jsx-1144101709'
        >
          <defs className='jsx-1144101709'>
            <polygon
              id='path-1'
              points='0 0.3672 33.089 0.3672 33.089 21.3842 0 21.3842'
              className='jsx-1144101709'
            />
            <polygon
              id='path-3'
              points='0 0.00039 22.3713346 0.00039 22.3713346 5.69739 0 5.69739'
              className='jsx-1144101709'
            />
          </defs>
          <g
            id='Page-1'
            fill='none'
            fillRule='evenodd'
            className='jsx-1144101709'
          >
            <g id='Cart-Full' className='jsx-1144101709'>
              <g
                id='Page-1'
                transform='translate(0 1)'
                className='jsx-1144101709'
              >
                <g
                  id='Group-3'
                  transform='translate(0 .633)'
                  className='jsx-1144101709'
                >
                  <mask id='mask-2' fill='#fff' className='jsx-1144101709'>
                    <use xlinkHref='#path-1' className='jsx-1144101709' />
                  </mask>
                  <path
                    d='M8.4296,4.5812 L11.5266,20.1472 L28.3366,19.1582 L31.7276,4.4012 L20.0506,4.4012 L8.4296,4.5812 Z M11.0356,21.3842 C10.7506,21.3842 10.5006,21.1822 10.4446,20.8982 L7.1386,4.2782 L4.5696,1.5742 L0.6036,1.5742 C0.2706,1.5742 -0.0004,1.3042 -0.0004,0.9712 C-0.0004,0.6372 0.2706,0.3672 0.6036,0.3672 L4.8286,0.3672 C4.9946,0.3672 5.1526,0.4352 5.2666,0.5552 L7.9516,3.3812 L20.0416,3.1942 L32.4856,3.1942 C32.6696,3.1942 32.8436,3.2782 32.9576,3.4222 C33.0726,3.5662 33.1156,3.7542 33.0736,3.9332 L29.4116,19.8692 C29.3516,20.1302 29.1266,20.3212 28.8596,20.3362 L11.0726,21.3832 C11.0606,21.3842 11.0486,21.3842 11.0356,21.3842 Z'
                    id='Fill-1'
                    fill='#231F20'
                    mask='url(#mask-2)'
                    className='jsx-1144101709'
                  />
                </g>
                <path
                  d='M29.4276,24.4718 L11.1166,24.4718 C10.7836,24.4718 10.5126,24.2008 10.5126,23.8678 L10.5126,21.4128 C10.5126,21.0798 10.7836,20.8098 11.1166,20.8098 C11.4496,20.8098 11.7206,21.0798 11.7206,21.4128 L11.7206,23.2638 L29.4276,23.2638 C29.7606,23.2638 30.0316,23.5348 30.0316,23.8678 C30.0316,24.2008 29.7606,24.4718 29.4276,24.4718'
                  id='Fill-4'
                  fill='#231F20'
                  className='jsx-1144101709'
                />
                <path
                  d='M14.4169,27.2785 C14.4169,28.5785 13.3629,29.6325 12.0629,29.6325 C10.7629,29.6325 9.7089,28.5785 9.7089,27.2785 C9.7089,25.9785 10.7629,24.9245 12.0629,24.9245 C13.3629,24.9245 14.4169,25.9785 14.4169,27.2785'
                  id='Fill-6'
                  fill='#231F20'
                  className='jsx-1144101709'
                />
                <path
                  d='M30.2723,27.2785 C30.2723,28.5785 29.2183,29.6325 27.9183,29.6325 C26.6183,29.6325 25.5643,28.5785 25.5643,27.2785 C25.5643,25.9785 26.6183,24.9245 27.9183,24.9245 C29.2183,24.9245 30.2723,25.9785 30.2723,27.2785'
                  id='Fill-8'
                  fill='#231F20'
                  className='jsx-1144101709'
                />
              </g>
              <g
                id='Page-1'
                transform='translate(9)'
                className='jsx-1144101709'
              >
                <mask id='mask-4' fill='#fff' className='jsx-1144101709'>
                  <use xlinkHref='#path-3' className='jsx-1144101709' />
                </mask>
                <path
                  d='M1.2421,5.69759 C1.6261,4.52259 2.7141,3.71459 3.9671,3.71459 C4.2391,3.71459 4.5281,3.76559 4.8471,3.86959 C5.1271,3.96059 5.4341,3.83559 5.5701,3.57359 C6.0701,2.61059 7.0441,2.01259 8.1121,2.01259 C8.8161,2.01259 9.4981,2.27959 10.0341,2.76559 C10.1691,2.88659 10.3521,2.94159 10.5301,2.91459 C10.7081,2.88759 10.8661,2.78159 10.9591,2.62659 C11.4861,1.73859 12.4081,1.20759 13.4241,1.20759 C14.7071,1.20759 15.8421,2.07859 16.1851,3.32459 C16.2331,3.50059 16.3571,3.64359 16.5231,3.71759 C16.6901,3.79159 16.8791,3.78559 17.0401,3.70359 C18.7811,2.81759 20.7911,3.94759 21.1431,5.69759 L22.3671,5.69759 C22.3671,5.67659 22.3741,5.65559 22.3701,5.63459 C22.0681,3.66059 20.3331,2.17359 18.3341,2.17359 C17.9221,2.17359 17.5141,2.23859 17.1151,2.36859 C16.4551,0.94659 15.0201,-0.00041 13.4241,-0.00041 C12.2031,-0.00041 11.0801,0.53559 10.3101,1.45859 C9.6541,1.03359 8.8931,0.80459 8.1121,0.80459 C6.7561,0.80459 5.5081,1.47959 4.7511,2.59359 C4.4821,2.53559 4.2231,2.50759 3.9671,2.50759 C2.0891,2.50759 0.4591,3.77959 0.0041,5.59959 C-0.0039,5.63259 0.0021,5.66459 0.0001,5.69759 L1.2421,5.69759 Z'
                  id='Fill-1'
                  fill='#231F20'
                  mask='url(#mask-4)'
                  className='jsx-1144101709'
                />
              </g>
            </g>
          </g>
        </svg>
        <span style={{ top: '13%', left: '48%' }} className='text-sm absolute '>
          {count}
        </span>
      </div>
    </div>
  );
}

const mapStateToProps = (state) => ({ count: state.cart.data.length });

const mapDispatchToProps = (dispatch) => ({
  openCart: () => dispatch(openCart()),
});

export default connect(mapStateToProps, mapDispatchToProps)(Cart);
